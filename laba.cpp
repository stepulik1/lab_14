...
int main() {
    ...
    std::cout << "\n\n----------------------------------------------------------------------\n";
    std::cout << "1. Реализовать контейнер с итератором на основе базы данных SQLite для выбранного семейства объектов.\n";
    std::cout << "----------------------------------------------------------------------\n\n";
    ...
    std::cout << "\n\n----------------------------------------------------------------------\n";
    std::cout << "2. Реализовать взаимодействие с объектами выбранного семейства с использованием паттерна Стратегия. Интерфейс существующих функций менять запрещено!\n";
    std::cout << "----------------------------------------------------------------------\n\n";
    ...
    std::cout << "\n\n----------------------------------------------------------------------\n";
    std::cout << "3. Для выбранного семейства объектов реализовать паттерн Шаблонный метод (добавит некоторую функцию взаимодейтсвия с объектом, алгоритм которой является общим для всех объектов, но может иметь специфические реализации отдельных этапов для каждого конкретного объекта из семейства).\n";
    std::cout << "4. Добавить (заменить) использование интеллектуальных указателей на объекты.\n";
    std::cout << "----------------------------------------------------------------------\n\n";


    // Использование std::shared_ptr для замены обычных указателей. 
    // Главный архив теперь владеет объектами совместно.
    std::vector<std::shared_ptr<Publication2>> mainArchive;

    mainArchive.push_back(std::make_shared<Book2>(101, "Гарри Поттер", "Дж.К. Роулинг"));
    mainArchive.push_back(std::make_shared<Magazine2>(202, "Наука и Жизнь", "Выпуск №9"));
    mainArchive.push_back(std::make_shared<Book2>(103, "Изучаем C++", "Б. Страуструп"));

    // Демонстрация Задания 3 (Шаблонный метод через умные указатели)
    std::cout << "=== ЗАПУСК ПАКЕТНОЙ АРХИВАЦИИ БИБЛИОТЕКИ ===\n";
    for (const auto& pub : mainArchive) {
        pub->archivePublication();
    }

    // Использование std::weak_ptr для безопасного наблюдения.
    // Создаем список чтения. Он не владеет объектами, а только наблюдает за ними.
    std::vector<std::weak_ptr<Publication2>> readingList;

    // Добавляем первые две публикации в список чтения
    readingList.push_back(mainArchive[0]); // Гарри Поттер
    readingList.push_back(mainArchive[1]); // Наука и Жизнь

    std::cout << "\n=== РАБОТА СО СПИСКОМ ЧТЕНИЯ (weak_ptr) ===\n";
    auto printReadingList = [&readingList]() {
        std::cout << "[СПИСОК ЧТЕНИЯ]:\n";
        for (size_t i = 0; i < readingList.size(); ++i) {
            // Трансформация weak_ptr в shared_ptr для безопасного доступа
            if (std::shared_ptr<Publication2> safe_pub = readingList[i].lock()) {
                std::cout << "  " << i + 1 << ". Доступно: " << safe_pub->getTitle() << "\n";
            } else {
                // Если объект был удален из mainArchive, программа не упадет
                std::cout << "  " << i + 1 << ". [ОШИБКА]: Объект был списан из библиотеки и больше не доступен!\n";
            }
        }
    };

    // Выводим список (Оба объекта живы)
    printReadingList();

    // Демонстрация автоматического освобождения памяти.
    std::cout << "\n[СИСТЕМА] Списываем книгу 'Гарри Поттер' из главного архива...\n";
    // Удаляем объект из владеющего вектора (счетчик ссылок падает до 0, объект удаляется из памяти)
    mainArchive.erase(mainArchive.begin()); 

    // Снова проверяем список чтения. Программа безопасно определит, что памяти больше нет.
    printReadingList();
